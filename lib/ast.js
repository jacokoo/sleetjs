...
class TransformValue extends SleetValue {
    constructor(value, transformers, end, location) {
...
class Attribute extends NullableNamedNode {
    ...
    merge(other) {
        if (!this.name || this.name !== other.name || this.namespace !== other.namespace)
            return false;
...
class AttributeGroup extends SleetNode {
    ...
    merge(other, ignoreSetting = false) {
        if (!ignoreSetting && (other._setting || this._setting))
            return false;
...
class Tag extends NullableNamedNode {
    ...
    _setGroup(groups) {
        this._attributeGroups = groups.reduce((acc, item) => {
            if (!acc.length)
                return [item];
            if (acc[acc.length - 1].merge(item))
                return acc;
            return acc.concat(item);
        }, []);
    }
